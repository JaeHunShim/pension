...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
	...
	_getPhotoTag : function(htPhotoInfo){
		...
		/*
		if(htPhotoInfo.nWidth > 600){
			
        nEditorWidth = jindo.$Element(this.oApp.getWYSIWYGDocument().body).width() ;
        nRatio = nEditorWidth / htPhotoInfo.nWidth;
        nResizeWidth = nRatio * htPhotoInfo.nWidth; 

		nResizeWidth = Math.round(Number(nRatio * htPhotoInfo.nWidth));
